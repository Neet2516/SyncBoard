...
export function BoardView() {
  ...
  if (error || !board) {
    return (
      <div className="flex h-screen items-center justify-center bg-[#f6efe3]">
        <div className="paper-card rounded-[2rem] p-10 text-center">
          <h1 className="section-title mb-2 text-2xl text-gray-800">Error</h1>
          <p className="section-copy mb-6 text-base">{error || 'Board not found'}</p>
          <Link to="/boards" className="font-semibold text-blue-600 hover:underline">
            Back to Dashboard
          </Link>
        </div>
      </div>
    ...
    <div className="flex h-screen flex-col overflow-hidden bg-[#f6efe3]">
      ...
      <nav className="z-10 flex h-20 shrink-0 items-center justify-between border-b border-white/50 bg-white/70 px-6 shadow-sm backdrop-blur-xl">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="hidden rounded-2xl bg-white/70 px-4 py-3 text-sm font-medium text-gray-500 shadow-sm lg:block">
            Room: <span className="font-mono text-xs text-gray-800">{board.boardId}</span>
          </div>
          {isOwner && (
            <button
...
function PresenceAvatars({ activeUsers }: { activeUsers: Array<{ name: string; color: string }> }) {
  return (
    <div className="flex items-center gap-2">
      <span className="stat-label text-xs text-gray-500">Active now</span>
        ...
        {activeUsers.length === 0 ? (
          ...
          activeUsers.map((user) => (
            <div
              ...
              className="flex h-9 w-9 items-center justify-center rounded-full border-2 border-white text-[11px] font-bold text-white shadow-md"
      ...
      <div className="paper-card w-full max-w-lg rounded-[2rem] bg-white/95 p-6">
        ...
        <div className="mb-6 space-y-3">
          <button
            ...
            className="w-full rounded-2xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm font-semibold text-blue-700 transition-colors hover:border-blue-300 hover:bg-blue-100"
          >
            Copy Invite Link
          </button>
        ...
        <div>
          <h3 className="stat-label mb-3 text-sm font-semibold uppercase text-gray-500">Editors</h3>
          <div className="space-y-3">
            {(board.collaborators ?? []).length === 0 ? (
              ...
              (board.collaborators ?? []).map((collaborator) => (
                <div key={collaborator.userId} className="flex items-center justify-between rounded-2xl border border-gray-200 bg-white/80 px-4 py-3">
                  <div>
